...
@FeignClient(name = "wxworkService",
             url = "${wxworkUrl}"
             // ,configuration=FeignClientConfiguration.class
             )
public interface WxworkService {
	@RequestMapping(method = RequestMethod.POST,
                    value = "/service/get_admin_list?suite_access_token={suiteAccessToken}",
                    consumes = "application/json")
	public GetAdminListRes getAdminList(@PathVariable("suiteAccessToken") String suiteAccessToken,
                                        @RequestBody GetAdminListReq getAdminListReq);
}
